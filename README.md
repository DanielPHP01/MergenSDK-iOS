...
- `Mergen` — one binaryTarget, downloaded as a checksummed zip from this
  repo's GitHub Releases on first resolve. The Swift wrapper is precompiled,
  and the C++ core, OpenCV, ONNX Runtime and OpenSSL are linked statically
  inside it. Your target needs no `linkerSettings` for any of them (versions
  through 2.1.x required nine).
...
2. Initialize / update this directory as its own git repo and push it to
   ...
   ```
3. Create the GitHub Release for the tag and attach the zip asset from
   `dist/release-assets/`:
   ```bash
