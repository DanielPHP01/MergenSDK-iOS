...
// Package.swift
dependencies: [
    .package(url: "https://github.com/DanielPHP01/MergenSDK-iOS", from: "2.5.1")
]
// target dependency:
...
`ScanIdCards` repo. To cut version `2.5.1`:

1. Generate the package (already done to produce this tree):
   ```bash
   SDK_VERSION=2.5.1 ./scripts/release_ios.sh
   ```
2. Initialize / update this directory as its own git repo and push it to
   ...
   git add -A
   git commit -m "Mergen iOS SDK v2.5.1"
